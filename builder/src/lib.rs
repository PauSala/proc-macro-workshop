...
fn extend_method(f: &syn::Field) -> Option<(bool, proc_macro2::TokenStream)> {
    ...
    let meta = match g.parse_args::<syn::Meta>() {
        Ok(syn::Meta::NameValue(nv)) => {
            if nv.path.get_ident().expect("An ident should exist") != "each" {
    ...
    match meta {
        syn::Expr::Lit(exp_lit) => match exp_lit.lit {
            syn::Lit::Str(s) => {
            ...
            _ => panic!("expected string, found {:?}", exp_lit),
        },
        _ => return mk_err(meta),
    }
}
